...
foreach var in Growth Lev ROA Age Size Cash RD_Ratio Invest TobinQ{
    capture drop l_`var'
    gen l_`var' = L.`var'
}
...
* 缩尾
local vars Invest l_Growth l_Lev l_ROA l_Age l_Size l_Invest l_Cash l_RD_Ratio l_TobinQ
winsor2 `vars', replace cuts(1 99)


local vars "Size Age Cash RD_Ratio Lev Growth Cashflow ADM Board IND"
foreach v in `vars' {
...
reghdfe Invest l_Growth l_Lev l_ROA l_Age l_Size l_Invest l_Cash l_RD_Ratio l_TobinQ, absorb(industry_id#year) residuals(resid)
...
local base_vars OverInvest N_Integration Integration ln_dte ln_dtu ln_total_patents ld hd ln_high_low ln_sub sub_asset Size Age Lev Cashflow RDSpendSumRatio Growth ADM Board IND
...
reghdfe sub_asset N_Integration Size Age Cashflow RDSpendSumRatio Lev Growth ADM Board IND, absorb(stkcd year) vce(cluster stkcd)
...
/* ===================================================
   异质性分析：链主 vs 非链主
   逻辑：链主企业（PageRank高）能有效抑制过度投资，体现赢家通吃红利
   =================================================== */
...
* 1. 确定划分标准：行业-年份内的 PageRank 前 10%
* 注意：需要确保 page_rank 变量在数据中（之前Gephi导入时有）
* 计算分位数
bysort ind_id year: egen p90_pr = pctile(page_rank), p(90)

* 生成虚拟变量：1=链主，0=非链主
gen Is_Leader = (page_rank >= p90_pr)
label var Is_Leader "是否链主企业"
...
* 2. 分组回归
* 组1：非链主企业 (预期：不显著或系数变大，体现战略性投资)
reghdfe OverInvest N_Integration Size Age Cashflow RDSpendSumRatio Lev Growth ADM Board IND if Is_Leader == 0, ///
    absorb(stkcd year) vce(cluster stkcd)
est store Group_NonLeader

* 组2：链主企业 (预期：显著为负，抑制过度投资)
reghdfe OverInvest N_Integration Size Age Cashflow RDSpendSumRatio Lev Growth ADM Board IND if Is_Leader == 1, ///
    absorb(stkcd year) vce(cluster stkcd)
est store Group_Leader

* 查看两组核心变量的均值和方差
bysort Is_Leader: sum Integration

/* 解读：
   关注 c.N_Integration#1.Is_Leader 的系数。
   如果显著为负，说明链主身份"强化"了融入度对过度投资的抑制作用。
*/  
...
/* ===================================================
   异质性分析：高敏捷组企业 vs 非高敏捷组企业
   逻辑：高敏捷组企业（Closeness高）会缓解过度投资
   =================================================== */

* 1. 确定划分标准：行业-年份内的 Closeness 前 50%
* 注意：需要确保 Closeness 变量在数据中（之前Gephi导入时有）

* 计算分位数
bysort ind_id year: egen p50_pr_cl = pctile(closeness), p(50)

* 生成虚拟变量：1=高敏捷组企业，0=非高敏捷组企业
gen Group_HighClose = (closeness >= p50_pr_cl)
label var Group_HighClose "是否高敏捷组企业"
...
* 2. 分组回归
* 组1：非高敏捷组企业 (预期：不显著或系数变大，体现战略性投资)
reghdfe OverInvest N_Integration Size Age Cashflow RDSpendSumRatio Lev Growth ADM Board IND if Group_HighClose == 0, ///
    absorb(stkcd year) vce(cluster stkcd)
est store Group_LowClose

* 组2：高敏捷组企业 (预期：显著为负，抑制过度投资)
reghdfe OverInvest N_Integration Size Age Cashflow RDSpendSumRatio Lev Growth ADM Board IND if Group_HighClose == 1, ///
    absorb(stkcd year) vce(cluster stkcd)
est store Group_HighClose

* 查看两组核心变量的均值和方差
bysort Group_HighClose: sum N_Integration

/* 解读：
   关注 c.N_Integration#1.Group_HighClose 的系数。
   如果显著为负，说明高速的信息获取"强化"了融入度对过度投资的抑制作用。
*/ 
...
reghdfe INV_Biddle l_Growth l_RD_Ratio, absorb(industry_id#year) residuals(resid_biddle)
...
* 假设你刚才跑出来的真实当期系数依然是 -0.0888，如果变了请修改这里！
local true_coef = -0.08792 
...
twoway (kdensity beta_fake, bwidth(0.008) color(gs12) recast(area)), /// 
       ...
       legend(off) ///
       scheme(s1mono) ///
       note("Note: The red dashed line indicates the true baseline coefficient (-0.08792).")
